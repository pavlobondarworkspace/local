    ...
    <body>
        ...
        <script>
            async function handleGlobalMapInteraction(latitude, longitude) {{
                ...
                if (!loc1IsSet) {{ targetLoc = 1; confirmQuestion = `Set Location 1: (${{latitude}}, ${{longitude}})?`; }}
                else if (!loc2IsSet) {{ targetLoc = 2; confirmQuestion = `Set Location 2: (${{latitude}}, ${{longitude}})?`; }}
                else {{ Swal.fire('Selection Full', 'Both locations set. Reset to change.', 'info'); return; }}
